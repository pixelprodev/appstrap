...
class FixtureManager {
  ...
  load () {
    ...
    rootDir.forEach(file => {
      const fixtureName = file.replace('.js', '').toLowerCase()
  ...
  validateFixture (fixtureName, fixtureData) {
    const fixtureSchema = Joi.object({ injectState: Joi.function(), routes: Joi.array() }).min(1)
  ...
  deactivateFixture (fixtureName) {
    ...
    Array.from(handlerKeys).forEach(handlerKey => {
      const handler = this.handlers.pick(...handlerKey.split(':::'))
      handler.fixtures.delete(fixtureName)
    })
  }
}
