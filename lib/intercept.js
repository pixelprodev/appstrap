...
async function intercept (context, config) {
  ...
  let handler
  const forwardingDestination = getForwardingDestination(context, stateConfig.hostMap)
  if (forwardingDestination && matchingEndpoint.requestForwardingEnabled) {
  ...
  const { statusCode, headers, payload } = normalizeResponse(handler(context))
...
function getForwardingDestination (context, hostMap) {
  for (const pathToForward of Object.keys(hostMap)) {
    if (pathToForward === '*') {
      return hostMap[pathToForward]
    }
    const matcher = new RegExp(pathToForward)
    if (matcher.test(context.req.path)) {
      return hostMap[pathToForward]
    }
  }
}
