...
class Appstrap extends express {
  constructor ({
    ...
    if (gqlEndpoint) {
      ...
      this.use(gqlEndpoint, GraphQL.handleGqlRequest.bind(this))
    }
...
function configureRoutes () {
  ...
  const addFixtureInterceptor = (req, res, next) => {
    const defaultResJson = res.json
    res.json = (payload) => defaultResJson.call(res, this.fixtures.applyFixtures(req, payload))
    next()
  }
  const handleRoutes = (req, res, next) => this.routes(req, res, next)
  this.use(addFixtureInterceptor)
