...
class Interactor {
  ...
  getStatus (args, res) {
    ...
    Array.from(this.fixtures.active.keys()).forEach((activeFixtureName, indexOrder) => {
  ...
  setHandlerEnabled ({ path, method, operation, enabled }, res) {
    const handler = this.handlers.pick({ path, method, operation })
    handler.disabled = !enabled
    if (res) { res.sendStatus(200) }
  }

  setModifier ({ path, method, operation, ...args }, res) {
    const handler = this.handlers.pick({ path, method, operation })
