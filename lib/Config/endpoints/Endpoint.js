...
class Endpoint {
  ...
  async execute (context, fixtures, fn, modifiers) {
    ...
    if (modifiers.responseSequence.length > 0) {
      ...
      if (modifiers.forwardRequest) {
        payload = forwardRequest(context)
      } else {
        payload = fn(context)
      }
    }
...
class GraphEndpoint extends Endpoint {
  ...
  async execute (context, fixtures) {
    ...
    if (!operationName || !this.operations.has(operationName)) { return context.next() }
    const modifiers = this.modifiers.get(operationName)
