...
class Config {
  constructor (configArgs) {
    this.load(configArgs)
    configArgs.events.on('watcher:change', () => this.reload(configArgs))
  }
  ...
  load ({ repository, gqlEndpoint, events }) {
    ...
    const hostMap = this.files.find(file => file === `${this.repository}/hostMap.js`)
    this.hostMap = hostMap ? loadFile(hostMap) : {}

    this.fixtures = new Fixtures({ files: this.files, events })
    this.endpoints = endpoints.initialize({ files: this.files, gqlEndpoint, events, hostMap: this.hostMap })
    events.emit('config:ready', this)
  }

  reload (configArgs) {
    configArgs.events.emit('log', { level: 'info', message: 'A change in your configuration was detected - reloading now.' })
    // todo theres things to fix here
    const activatedFixtures = this.fixtures.active
