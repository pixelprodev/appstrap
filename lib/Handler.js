...
class Handler {
  constructor ({ alias, filePath, method, fn }) {
    this.path = filePath.replace(/.*routes\//, '/').replace('.js', '')
    this.id = alias || `${this.path}:::${method.toUpperCase()}`
    this.fn = fn
  ...
  applyFixtures (data) {
    ...
    return Array.from(this.fixtures.values()).reduce((updatedData, fixture) => fixture.execute(data), data)
  }
